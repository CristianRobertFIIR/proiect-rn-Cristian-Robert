...
**Link Repository GitHub:** [https://github.com/CristianRobertFIIR/ProiectRecunoasterePasariRN/edit/main/README_Etapa4_Arhitectura_SIA_03.12.2025.md]  
...
```
Justificare Batch Size = 32:
Aceasta valoare este considerata standardul optim ("sweet spot") in Deep Learning din doua motive principale
   1. Eficienta Hardware: Fiind o putere a lui 2 ($2^5$), numarul 32 optimizeaza alinierea memoriei si calculele matriciale pe GPU, fara a depasi memoria VRAM disponibila.
   2. Stabilitate si Generalizare: Ofera un echilibru perfect intre viteza de antrenare si calitatea invatarii. Introduce suficient "zgomot" statistic pentru a ajuta modelul sa nu se blocheze in minime locale (evitand overfitting-ul specific batch-urilor mari), pastrand totodata o convergenta stabila a gradientului.
```
...
```
Matricea de Confuzie indica o rata de eroare ridicata (aprox. 15-20%) intre speciile cu penaj si morfologie similara, in special intre pasarile acvatice precum '024.Red_faced_Cormorant' si '008.Rhinoceros_Auklet'.
Cauza posibila: La rezolutia de intrare de 224x224 pixeli, detaliile fine care diferentiaza aceste specii (forma ciocului, nuante subtile ale penajului) se pierd, iar reteaua neuronala se bazeaza excesiv pe silueta generala, care este aproape identica la cele doua specii.
```
...
**Completați pentru proiectul vostru:**
```
Analiza erorilor a evidentiat doi factori vizuali critici care scad increderea modelului:
1. Fundalul complex (Noise): Imaginile in care pasarea se afla in vegetatie densa (crengi, frunze) duc la erori de segmentare, deoarece modelul considera elementele de fundal ca facand parte din pasare.
2. Variatia de iluminare: Fotografiile subexpuse (intunecate) sau cu umbre puternice ascund texturile specifice ale penajului. Deoarece MobileNetV2 este un model care se bazeaza mult pe texturi pentru clasificare, lipsa contrastului duce frecvent la clasificari gresite sau la scoruri de incredere sub pragul de validare.
```
...
**Completați pentru proiectul vostru:**
```
FALSE NEGATIVES (specie nedetectata): CRITIC -> In contextul monitorizarii de mediu, neidentificarea unei specii protejate duce la rapoarte ecologice incomplete si pierderea datelor despre biodiversitate.
FALSE POSITIVES (alarma falsa): ACCEPTABIL -> Identificarea gresita a unei pasari comune drept specie rara genereaza alerte inutile, dar acestea pot fi filtrate ulterior manual de un ornitolog.

Prioritate: Minimizare False Negatives (preferam sa avem alerte false decat sa pierdem prezenta unei specii rare).
Solutie: Ajustarea pragului de incredere (threshold) de la 0.5 la 0.3 pentru speciile considerate critice/protejate.
```
...
```
Masuri corective:
1. Fine-tuning avansat: "Dezghetarea" (unfreezing) ultimelor 30 de straturi ale modelului MobileNetV2 si re-antrenarea cu o rata de invatare foarte mica (1e-5) pentru a permite modelului sa invete detalii fine ale penajului, nu doar forme generale.
2. Augmentare pentru mediu natural: Implementarea unor transformari specifice de contrast si luminozitate (RandomBrightnessContrast) pentru a simula conditii de lumina slaba sau umbre puternice, frecvente in padure.
3. Colectare tintita de date: Adaugarea a min. 200 de imagini suplimentare strict pentru perechile de clase confundate frecvent (ex: Cormoran vs Auklet) pentru a echilibra setul de date si a oferi retelei mai multe exemple negative.
```
